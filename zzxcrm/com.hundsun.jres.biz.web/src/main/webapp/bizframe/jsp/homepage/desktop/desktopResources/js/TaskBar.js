...
Ext.extend(Ext.ux.TaskBar, Ext.util.Observable, {
	init : function() {
		...
		var toolbar = [{
			//id : 'timePanel',
				...
				template : new Ext.Template(
						...
						'<td class="x-btn-mc ux-taskbutton-center"><em class="{5} unselectable="on">',
				...
				template : new Ext.Template(
						...
						'<td class="ux-taskbutton-left"><i>&#160;</i></td>',
						'<td class="x-btn-mc ux-taskbutton-center"><em class="{5} unselectable="on">',
						'<button class="x-btn-text {2}" type="{1}" style="height:30px;border:0">{0}</button>',
						'</em></td>',
						'<td class="ux-taskbutton-right"><i>&#160;</i></td>',
						"</tr></tbody></table>"),
				handler : function() {
		...
		this.timePanel = new Ext.Panel({
			//el:'ux-statusbar-panel',
			frame : false,
			border : false,
			bodyBorder : false,
			id:'statusbar',
			region : 'east',
			bodyStyle:'background:#000000 url( bizframe/jsp/homepage/desktop/desktopResources/images/taskbar/black/taskbuttons-panel-bg.gif ) repeat-x left top;',
			width : 335,
			height:30,
			items : toolbar,
			listeners:{
			  beforerender:function(panelObj){
			   //this.el.child('div.x-panel-body x-panel-body-noheader x-panel-body-noborder'+);
			  	//var nodes = Ext.DomQuery.selectNode( "",panelObj);
			  }
			}
		});

		/*
		 * //增加锁屏、消息提示等 this.timePanel = new Ext.ButtonGroup({ id: 'timePanel',
		 * border:false, renderTo:'ux-time-panel', items:[{ xtype:'button',
		 * text: '锁屏', height:30, tooltip:'单击此处可锁定屏幕', id:'lockScreen',
		 * iconCls:'lock', template: new Ext.Template( '<table cellspacing="0"
		 * class="x-btn {3}"><tbody><tr>', '<td class="ux-taskbutton-left"><i>&#160;</i></td>', '<td class="ux-taskbutton-center"><em class="{5} unselectable="on">', '<button
		 * class="x-btn-text {2}" type="{1}" style="height:28px;">{0}</button>', '</em></td>', '<td class="ux-taskbutton-right"><i>&#160;</i></td>', "</tr></tbody></table>") ,
		 * handler:function(){ clearInterval(timerPool);//取消轮询 openLoginWin();
		 * loginWin.setTitle("屏幕已经锁定，请重新输入密码登录!"); }
		 * 
		 * },{ xtype:'button', text: '提示已开启', tooltip:'单击此处关闭消息提示', height:30,
		 * id:'handMsgNotice', iconCls:'openMsgNotice', template: new
		 * Ext.Template( '<table cellspacing="0" class="x-btn {3}"><tbody><tr>', '<td class="ux-taskbutton-left"><i>&#160;</i></td>', '<td class="ux-taskbutton-center"><em class="{5} unselectable="on">', '<button
		 * class="x-btn-text {2}" type="{1}" style="height:30px;border:0">{0}</button>', '</em></td>', '<td class="ux-taskbutton-right"><i>&#160;</i></td>', "</tr></tbody></table>") ,
		 * handler:function(){ handNotice(this);}
		 * 
		 * },{ id:'msgNotice', xtype:'button', height:30, text:'消息提示',
		 * iconCls:'msgNotice', template: new Ext.Template( '<table
		 * cellspacing="0" class="x-btn {3}"><tbody><tr>', '<td class="ux-taskbutton-left"><i>&#160;</i></td>', '<td class="ux-taskbutton-center"><em class="{5} unselectable="on">', '<button
		 * class="x-btn-text {2}" type="{1}" style="height:28px;">{0}</button>', '</em></td>', '<td class="ux-taskbutton-right"><i>&#160;</i></td>', "</tr></tbody></table>") ,
		 * handler:function(){ if(isMsgGet){ showNoticeWin(); }
		 * //intNoticeConfig(this); }
		 * 
		 * },{ xtype:'button', text: '在线人数', id:'onlineCountBtn', height:30,
		 * iconCls:'', template: new Ext.Template( '<table cellspacing="0"
		 * class="x-btn {3}"><tbody><tr>', '<td class="ux-taskbutton-left"><i>&#160;</i></td>', '<td class="ux-taskbutton-center"><em class="{5} unselectable="on">', '<button
		 * class="x-btn-text {2}" type="{1}" style="height:28px;">{0}</button>', '</em></td>', '<td class="ux-taskbutton-right"><i>&#160;</i></td>', "</tr></tbody></table>") ,
		 * handler:function(){getOnlineCount(); }
		 *  }
		 * 
		 *  ], width:700, height:40, region:'east' });
		 * 
		 * 
		 */
		...
		var container = new Ext.ux.TaskBarContainer({
			el : 'ux-taskbar',
			layout : 'border',
			items : [sbBox, this.tbPanel,this.timePanel]
		});
		return this;
	},
...
Ext.ux.TaskButtonsPanel = Ext.extend(Ext.BoxComponent, {
	...
	autoSize : function() {
		...
		var each = Math.max(Math.min(Math.floor((aw - 4) / count)
				- this.buttonMargin, this.buttonWidth), this.minButtonWidth); // -4
																				// for
																				// float
																				// errors
																				// in
...
Ext.extend(Ext.ux.TaskBar.TaskButton, Ext.Button, {
	onRender : function() {
		...
		// //////////
		var parWin = this.win;
		var tipHeight = 100, tipWidth = 165;
		// var heavyImage = new Image();
		// heavyImage.src ='../images/tip2.jpg';
		this.tipWindow = new Ext.ToolTip({
			anchor : 'bottom',
			autoHide : true,
			hideDelay : 300,
			height : tipHeight,
			...
			listeners : {
				afterrender : function() {
					...
					// var ctx = this.body.dom.getContext('2d');
					// ctx.drawImage(heavyImage, 0, 0, tipWidth, tipHeight );
				}
			}
		});
		this.tipWindow.initTarget(this.el);
